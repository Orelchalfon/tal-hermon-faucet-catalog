...
const configs = [
  {
    id: "stick-2",
    title: "סטיק 2 ידיות לקר וסודה",
    placeholder: "תמונת ברז מזיגה רגיל סטיק 2 ידיות לקר וסודה",
    description: "ברז סטיק קלאסי עם שתי ידיות נפרדות — אחת למים קרים מסוננים ואחת לסודה מוגזת ישירות מהברז.",
    benefits: ["שליטה מדויקת בכל סוג מים", "עיצוב סטיק נקי ומינימליסטי", "מתאים לכל ארון מטבח תת־כיורי"],
  },
  {
    id: "stick-1-plus",
    title: "סטיק ידית אחת לסודה + ברז חם/קר",
    placeholder: "תמונת ברז מזיגה סטיק ידית אחת לסודה + ברז מזיגה חם קר",
    description: "פתרון מורחב — ברז סטיק ייעודי לסודה לצד ברז מזיגה נוסף למים חמים רותחים וקרים.",
    benefits: ["הפרדה בין מערכת הסודה למערכת המים", "מים רותחים זמינים מיידית", "חוויית שתייה מלאה במטבח אחד"],
  },
  {
    id: "all-in-one",
    title: "ברז אחד חם/קר/סודה",
    placeholder: "תמונה של ברז חם קר סודה בברז אחד",
    description: "ברז יחיד שמרכז את שלושת מקורות המים — קר מסונן, רותח וסודה מוגזת. מראה נקי וחסכון במקום על השיש.",
    benefits: ["ברז אחד בלבד על השיש", "חוויית משתמש פשוטה ואלגנטית", "מתאים למטבחים בעיצוב מודרני"],
  },
];
...
const finishes = [
  ...
  {
    name: "זהב מוברש",
    style: "repeating-linear-gradient(135deg,#d8b465 0 2px,#b88a35 2px 4px)",
  },
  { name: "ניקל", style: "linear-gradient(135deg,#e6e8ec,#9aa1ab)" },
  {
    name: "ניקל מוברש",
    style: "repeating-linear-gradient(135deg,#cfd3d8 0 2px,#9aa1ab 2px 4px)",
  },
    ...
    <header className="sticky top-0 z-30 border-b border-white/10 bg-brand-ink/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        <nav className="hidden gap-6 text-sm text-white/70 md:flex">
          <a href="#configs" className="hover:text-brand-aqua">
            קונפיגורציות
          </a>
          <a href="#specs" className="hover:text-brand-aqua">
            מפרט טכני
          </a>
          <a href="#finishes" className="hover:text-brand-aqua">
            גימורים
          </a>
          <a href="#contact" className="hover:text-brand-aqua">
            צור קשר
          </a>
        </nav>
    ...
    <section className="relative overflow-hidden bg-brand-navy text-white">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24">
        ...
        <div className="order-1 flex flex-col justify-center md:order-2">
          ...
          <h1 className="text-3xl font-bold leading-tight sm:text-4xl md:text-5xl">ברזי מזיגה למים קרים, חמים וסודה</h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#configs">
              <Button className="bg-brand-aqua text-brand-ink hover:bg-brand-aqua/90">לצפייה בקונפיגורציות</Button>
            </a>
            <a href="#contact">
    ...
    <section id="configs" className="bg-brand-mist py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-12 max-w-2xl">
          <h2 className="text-2xl font-bold text-brand-navy sm:text-3xl">קונפיגורציות הברז</h2>
          <p className="mt-3 text-brand-navy/70">שלוש תצורות שונות שמותאמות לכל סגנון מטבח ולכל הרגל שתייה.</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {configs.map((c) => (
            ...
            >
              ...
              <h3 className="mt-5 text-lg font-bold text-brand-navy">{c.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-brand-navy/70">{c.description}</p>
              <ul className="mt-5 space-y-2 border-t border-brand-navy/10 pt-4">
                {c.benefits.map((b) => (
                  <li key={b} className="flex items-start gap-2 text-sm text-brand-navy/80">
                    ...
                    <span>{b}</span>
                    <p>* יש אפשרות לפיקוד שבת בפיקוח משמרת השבת</p>
                  </li>
                ))}
              </ul>
            </article>
  ...
  return (
    <div className="flex h-full flex-col rounded-3xl border border-brand-navy/10 bg-white p-6">
      ...
      <dl className="divide-y divide-brand-navy/10">
        {specs.map((s) => (
          <div key={s.label} className="flex items-start justify-between gap-4 py-3 text-sm">
            <dt className="font-medium text-brand-navy">{s.label}</dt>
            <dd className="text-left text-brand-navy/70">{s.value}</dd>
          </div>
        ))}
      </dl>
      {footnote && <p className="mt-4 rounded-xl bg-brand-mist p-3 text-xs text-brand-navy/70">{footnote}</p>}
    </div>
  );
}
...
function Specs() {
  return (
    <section id="specs" className="bg-white py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-12 max-w-2xl">
          <h2 className="text-2xl font-bold text-brand-navy sm:text-3xl">מפרט טכני</h2>
          <p className="mt-3 text-brand-navy/70">המערכת התת־כיורית מורכבת משלושה רכיבים שעובדים יחד בשקט מתחת לשיש.</p>
        </div>
    ...
    <section id="finishes" className="bg-brand-mist py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-10 max-w-2xl">
          <h2 className="text-2xl font-bold text-brand-navy sm:text-3xl">גימורי ברז זמינים</h2>
          <p className="mt-3 text-brand-navy/70">כל ברז זמין במגוון גימורים שמשתלבים בעיצוב המטבח שלכם.</p>
        </div>
    ...
    <section id="contact" className="bg-brand-navy py-20 text-white">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <div className="mb-10 text-center">
          ...
          <h2 className="text-2xl font-bold sm:text-3xl">רוצים להתאים את הברז למטבח שלכם?</h2>
          <p className="mt-3 text-white/70">השאירו פרטים וניצור איתכם קשר עם המלצה מותאמת אישית.</p>
        </div>
        <form
          ...
          <div className="space-y-2">
            ...
            <Select value={form.interest} onValueChange={(v) => setForm({ ...form, interest: v })}>
              ...
              <SelectContent>
                ...
                <SelectItem value="לא בטוח/ה — צריך ייעוץ">לא בטוח/ה — צריך ייעוץ</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="sm:col-span-2">
            <Button type="submit" className="w-full bg-brand-aqua text-brand-ink hover:bg-brand-aqua/90" size="lg">
              קבלו שיחה מנציג
            </Button>
          </div>
        </form>
